...
function computeRawScore(record: EnhancedTeamRecord, leagueType: string): number {
...
export function detectNotableChanges(
  ...
  changes.slice(0, 5).forEach((change) => {
    if (change.type === "new") {
      narrative.push(`${change.team.teamName} enters rankings at #${change.team.rank}`);
    } else if (change.type === "jump") {
      const prevRank = change.team.previousRank;
      narrative.push(`${change.team.teamName} jumps from #${prevRank} to #${change.team.rank}`);
    } else if (change.type === "drop") {
      const prevRank = change.team.previousRank;
      narrative.push(`${change.team.teamName} falls from #${prevRank} to #${change.team.rank}`);
...
export function formatRankingsDisplay(rankings: WeeklyRankings, playerTeamId: string): string {
...
export function getUpsetAlerts(rankings: WeeklyRankings, weekResults: WeekResult[]): string[] {
  const alerts: string[] = [];
